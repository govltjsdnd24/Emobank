...
public class UserController {
    ...
    public ResponseEntity<? extends BaseResponseBody> createUser(
        ...
        if(user != null){
            return ResponseEntity.ok(BaseResponseBody.of(200,"ID Existed"));
        }
    ...
    public ResponseEntity<? extends BaseResponseBody> login(@RequestBody UserLoginPostReq loginInfo){
        ...
        if(user == null){
            return ResponseEntity.ok(BaseResponseBody.of(200,"ID Invalid"));
        }

        if(!passwordEncoder.matches(password, user.getPassword())){
            return ResponseEntity.ok(BaseResponseBody.of(200,"Passsword Invaild"));
        }
